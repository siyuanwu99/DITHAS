/**
 * @file
 * @brief receive detection pose messages and solve the tf from self to global
 *
 * @date 30/12/2023
 * @version 1.0.0
 ...
 */
void poseCallback(std::vector<TrajPoint>& buf, const geometry_msgs::PoseStamped& msg) {
  if (t0 < 0.1) {
...
void poseSelfCallback(const geometry_msgs::PoseStamped& msg) {
  poseCallback(buf_odom_self_, msg);
  std::cout << "local: " << msg.header.stamp.toSec() - t0 << " pos: " << msg.pose.position.x << " "
            << msg.pose.position.y << " " << msg.pose.position.z << std::endl;
}
void poseGlblCallback(const geometry_msgs::PoseStamped& msg) {
  std::cout << "global: " << msg.header.stamp.toSec() - t0 << " pos: " << msg.pose.position.x << " "
            << msg.pose.position.y << " " << msg.pose.position.z << std::endl;
  poseCallback(buf_odom_glbl_, msg);
  // geometry_msgs::PoseStamped offset_msg = msg;
  //
  // offset_msg.header.stamp = ros::Time(msg.header.stamp.toSec() + TEMPORAL_OFFSET);

  // poseCallback(buf_odom_glbl_, offset_msg);
  // std::cout << "offset global: " << offset_msg.header.stamp.toSec() - t0
  // << " pos: " << offset_msg.pose.position.x << " " << offset_msg.pose.position.y << " "
  // << offset_msg.pose.position.z << std::endl;
}
...
PointPairs getPointPairs(const std::vector<TrajPoint>& src, const std::vector<TrajPoint>& dst) {
  ...
  for (size_t i = 0; i < src.size(); ++i) {
    ...
    while (j < static_cast<int>(dst.size())) {
...
void solveCallback(const ros::TimerEvent& event) {
  ...
  if (aligned_points.size() < 5) {
    ...
    ROS_INFO("local: %lu, [%f -> %f]", buf_odom_self_.size(), buf_odom_self_.front().time - t0,
             buf_odom_self_.back().time - t0);
    ROS_INFO("global: %lu, [%f -> %f]", buf_odom_glbl_.size(), buf_odom_glbl_.front().time - t0,
             buf_odom_glbl_.back().time - t0);
    return;
  }
...
int main(int argc, char* argv[]) {
  ...
  /** load params */
  nh.param<double>("solve_time", SOLVE_TIME, 0.2);
  nh.param<double>("max_temporal_diff", MAX_TEMPORAL_DIFF, 0.050);
  nh.param<double>("temporal_offset", TEMPORAL_OFFSET, 0.0);

  /** subscribe tag detection odom msg */
  ros::Subscriber odom1_sub = nh.subscribe("tag_pose_self", 1, poseSelfCallback);
  ros::Subscriber odom2_sub = nh.subscribe("tag_pose_glbl", 1, poseGlblCallback);
